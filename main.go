...
func main() {
	db = utils.InitializeDatabase("my.db")
...
func RecieveMessage(sess *discordgo.Session, mess *discordgo.MessageCreate) {
	...
	if strings.ToLower(mess.Mentions[0].String()) == strings.ToLower(sess.State.User.String()) {
		...
		if utils.IgnoreCase(slice[1], "get") {
			if len(slice) == 5 {
				sess.ChannelMessageSend(mess.ChannelID, utils.GetBuild(returnDB(), slice[2], slice[3], slice[4]))
			} else if len(slice) == 4 {
				sess.ChannelMessageSend(mess.ChannelID, utils.GetBuild(returnDB(), slice[2], slice[3], ""))
			} else if len(slice) == 3 {
				sess.ChannelMessageSend(mess.ChannelID, utils.GetBuild(returnDB(), slice[2], "", ""))
			} else {
			...
			// 	return
			// }
			utils.SaveBuild(returnDB(), slice[2], slice[3], slice[4], slice[5:len(slice)], mess.Author.String())
			...
			s, r := utils.GetAllBuildCount(returnDB())
			...
			sess.ChannelMessageSend(mess.ChannelID, utils.GetRand(returnDB(), slice[2]))
		} else if utils.IgnoreCase(slice[1], "whitelist") && len(slice) > 2 && mess.Author.String() == "EnlistedReb#8778" {
			utils.AddToWhiteList(returnDB(), slice[2], mess.Author.String())
			...
			if !utils.CheckWhiteList(returnDB(), mess.Author.String()) {
			...
			sess.ChannelMessageSend(mess.ChannelID, utils.GetOneBuildId(returnDB(), slice[2]))
		} else if utils.IgnoreCase(slice[1], "delete") && len(slice) > 2 {
			if !utils.CheckWhiteList(returnDB(), mess.Author.String()) {
				sess.ChannelMessageSend(mess.ChannelID, "You don't have access to this command.")
				return
			}
			sess.ChannelMessageSend(mess.ChannelID, utils.DeleteBuild(returnDB(), slice[2]))
